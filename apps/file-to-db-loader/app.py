...
def truncate_table(db_conn_uri, ds_name):
    engine = create_engine(db_conn_uri)

    with engine.begin() as conn:
        conn.execute(text(f'TRUNCATE TABLE "{ds_name}"'))
...
def db_loader(src_base_dir, db_conn_uri, ds_name):
    ...
    for file in files:
        ...
        for idx, df in enumerate(df_reader):
            ...
            print(f'Populated chunk {idx} of {ds_name}')
...
def process_files(ds_names=None):
    ...
    for ds_name in ds_names:
        try:
            print(f'Deleting data in {ds_name} table')
            truncate_table(db_conn_uri, ds_name)
